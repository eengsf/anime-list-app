...
export default function AnimeList({ api }) {
   ...
   return (
      <div className="grid md:grid-cols-4 sm:grid-cols-3 grid-cols-2 gap-4 px-4">
         {/* tanda ? setelah data? itu berarti kalau memang ada datanya maka munculkan kalau tidak ya tunggu dulu aja */}
         {api?.data.map((anime, index) => {
            return (
               <Link href={`/anime/${anime.mal_id}`} className="cursor-pointer text-color-primary hover:text-color-accent transition-all" key={index}>
                  ...
                  />
                  <h1 className="font-bold text-md p-4 ">{anime.title}</h1>
               </Link>
            );
         })}
      </div>
